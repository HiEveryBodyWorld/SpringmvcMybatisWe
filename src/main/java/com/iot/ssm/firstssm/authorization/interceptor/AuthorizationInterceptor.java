package com.iot.ssm.firstssm.authorization.interceptor;
...
import com.iot.ssm.firstssm.authorization.annotation.Authorization;
import com.iot.ssm.firstssm.authorization.manager.TokenManager;
...
public class AuthorizationInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (token != null && token.startsWith(httpHeaderPrefix)
				&& token.length() > 0) {
		...
		if (method.getAnnotation(Authorization.class) != null // 查看方法上是否有注解
				|| handlerMethod.getBeanType().getAnnotation(
						Authorization.class) != null) { // 查看方法所在的Controller是否有注解
